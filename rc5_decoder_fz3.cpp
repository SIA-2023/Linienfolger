...
//	pinMode(7,OUTPUT);    
	...
	PORT_RC5 &= ~(1<<PIN_MASK_RC5_FZ3);  //Input
	EIFR   |=  (1<<INTF1);      // Reset Interrupt 2 Flag 
	EICRA |= (1<<ISC10); //Any edge of INT2 
	EIMSK |= (1<<INT1); //External Interrupt 2 enable
...
void rc5_read_fz3(unsigned char *toggle, unsigned char *address, unsigned char *command)
{	
	if(cnt_bits >= 28)
	{
		*command = rc5_command;
...
unsigned long period;
  ...
  if(period > TIME_OUT)
  {
    if(!(PIN_RC5 & PIN_MASK_RC5_FZ3))  //Start Bit Wurde erkannt        
    {                                 
          rc5_code =  0;    
          cnt_bits = 1;
    }
